...
const RAW: [string, number, number, number, number | null, number, number][] = [
...
const ROWS: PlayerRow[] = RAW.map(([name, app, made, avg, best, rounds, sgAvg]) => ({ name, app, made, avg, best, rounds, sgAvg }));
...
export default function CourseHistoryPage() {
  ...
  const rows = useMemo(() => {
    ...
    let filtered = ROWS.filter((r) => r.app >= minApp && r.name.toLowerCase().includes(q));
    ...
    <>
      <header>
        ...
        <div className="subline">
          Course history <Link href="/" className="admin-link">· back to board</Link>
        </div>
      </header>

      <main>
        <div className="card live" style={{ marginBottom: 20, padding: "14px 16px 16px" }}>
          <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 18, fontWeight: 700, color: "var(--gold-bright)", marginBottom: 4 }}>
            TPC Twin Cities · 3M Open
          </div>
          <div style={{ fontSize: 12, color: "var(--cream-dim)", lineHeight: 1.6, maxWidth: 720 }}>
            Every appearance from 2019-2025, reshaped around make-cut rate and finish average since that's what actually drives
            Top N / Make Cut parlays - strokes gained is kept as a secondary sort. Manually transcribed from a screenshot, so spot-check
            a name before wagering off it if it matters. <span style={{ color: "var(--cream)" }}>Kurt Kitayama</span> (100% cut rate, 3.5 avg finish,
            +2.99 SG/round across 2 apps) is the clear standout signal in the sheet. Scottie Scheffler isn't on this list at all - he's
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
          <input
            ...
            style={{
              ...
              fontFamily: "'JetBrains Mono',monospace", fontSize: 13, padding: "8px 12px", borderRadius: 4, minWidth: 220,
            }}
          />
          <select
            value={minApp}
            ...
            style={{
              background: "rgba(0,0,0,0.25)", border: "1px solid var(--line)", color: "var(--cream)",
              fontFamily: "'JetBrains Mono',monospace", fontSize: 12, padding: "7px 10px", borderRadius: 4,
            }}
          >
          ...
          <span style={{ fontSize: 11, color: "var(--cream-dim)", marginLeft: "auto" }}>{rows.length} players shown</span>
        </div>

        <div style={{ overflowX: "auto", border: "1px solid var(--line)", borderRadius: 6, background: "rgba(241,236,221,0.03)" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, fontFamily: "'JetBrains Mono',monospace" }}>
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.name}>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", color: "var(--cream)", fontWeight: 600, whiteSpace: "nowrap" }}>
                      {r.name}
                    </td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>{r.app}</td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>{r.made}</td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>
                    ...
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>{r.avg.toFixed(1)}</td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>{r.best === null ? "—" : r.best}</td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right" }}>{r.rounds}</td>
                    <td style={{ padding: "8px 12px", borderBottom: "1px solid var(--line)", textAlign: "right", color: r.sgAvg >= 0 ? "var(--live)" : "var(--clay)" }}>
        ...
        <div style={{ fontSize: 11, color: "var(--cream-dim)", lineHeight: 1.6, maxWidth: 720, marginTop: 16, marginBottom: 40 }}>
          "Avg finish" scores every missed cut as 100 (as given on the source sheet), so a single-appearance player with a miss will show a flat 100.0 -
          check Apps/Made before reading too much into that number alone. "Best finish" excludes missed-cut appearances.
        </div>
      </main>
    </>
  );
